    ...
    <>
      ...
      <header className={`header ${scrollHeader ? 'scroll-header' : ''}`}>
        <nav className='nav container'>
          <div className="nav-container-itens"> 
            <Link to='/' className='nav-logo'>
              Flic <img src='/images/logo.svg' className='nav-logo-img' style={{ width: 15, marginLeft: 3 }} />
            </Link>
        ...
        {showSearch && (
          <div className='header-searchbar'>
            <div className='searchbar-container container' >
              ...
              />
              <button className="header-button-search" onClick={() => navigate(`/search/${query}`)}><FiSend /></button>
              <IoIosCloseCircle className='search-close' onClick={toggleSearch} style={{width: 30}}/>
            </div>
          </div>
        )}
      </header>
